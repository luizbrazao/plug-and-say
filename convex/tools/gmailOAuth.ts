...
export const getAuthUrl = internalAction({
    ...
    handler: async (ctx, args) => {
        ...
        const stateSecret =
            (cfg.stateSecret as string | undefined) ||
            (process.env.GMAIL_STATE_SECRET as string | undefined);
...
export const callback = httpAction(async (ctx, req) => {
    ...
    if (!decoded?.departmentId) return new Response("Invalid state", { status: 400 });
